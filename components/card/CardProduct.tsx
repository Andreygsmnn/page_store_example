    ...
    return (
        <div className={styles.cardWrapper}>
            ...
            <div className={styles.imageContainer}>
                {product.rating.count > 299 && <div className={styles.hits}><span>Хит</span></div>}
            ...
            <div className={styles.content}>
                ...
                <div className={styles.price}><strong>{`${Math.round(product.price * 70)} `}</strong><span>/шт.</span></div>
                <div className={styles.footerCard}>
                    <div className={styles.buttonContainer}>
                        <button className={isProductInCart ? styles.cardButtonOnCart : styles.cardButtonOffCart}
                            onClick={() => !isProductInCart && addItem(product)}
                        >
                            {isProductInCart ? 'В корзине' : 'В корзину'}</button>
                        <div className={styles.buttonToggle}>
                            <button onClick={() => setCount(count - 1)}><Image src={minus} width={10} height={2} alt='minus-icon' /> </button>
                            <div> {count} </div>
                            <button onClick={() => setCount(count + 1)}> <Image src={plus} width={10} height={10} alt='plus-icon' /> </button>
                        </div>
                    </div>
                    <button className={styles.faivorites} onClick={addToFaivorites}>
                        <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="20" cy="20" r="19.5" stroke="#F5F5F7" />
                            <path d="M20 18C20 15.7909 18.2091 14 16 14C13.7909 14 12 15.7909 12 18C12 22 18 26 20 28" stroke="#999999" stroke-linecap="round" />
